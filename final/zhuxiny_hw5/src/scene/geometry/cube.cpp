...
glm::vec3 Cube:: SetRandomPoint(glm::vec3& local_p,float x,float y)//(0.0,12.0)
{
    int face = x *12;
    float a=x-0.5;
    float b=y-0.5;
    glm::vec3 cube[12] = {
        ...
        glm::vec3 point = glm::vec3(transform.T()*glm::vec4(cube[face],1));
        return point;
}
    ...
    if(t_n < t_f)
        ...
        if(t_final >= 0)
        {
            ...
            if((normal == glm::vec3(0,0,1)) || (normal == glm::vec3(0,0,-1))){
                result.tangent = glm::vec3(transform.invTransT()*glm::vec4(1,0,0,0));
                result.tangent = glm::normalize(result.tangent);
                result.bitangent = glm::vec3(transform.invTransT()*glm::vec4(0,1,0,0));
                result.bitangent = glm::normalize(result.bitangent);
            }
            else if((normal == glm::vec3(0,1,0)) || (normal == glm::vec3(0,-1,0))){
                result.tangent =glm::vec3(transform.invTransT()*glm::vec4(0,0,1,0));
                result.tangent =glm::normalize(result.tangent);
                result.bitangent = glm::vec3(transform.invTransT()*glm::vec4(1,0,0,0));
                result.bitangent = glm::normalize(result.bitangent);
            }
            else{
                result.tangent = glm::normalize(glm::vec3(transform.invTransT()*glm::vec4(0,1,0,0)));
                result.bitangent = glm::normalize(glm::vec3(transform.invTransT()*glm::vec4(0,0,1,0)));
            }
        }
        else{
            return result;
