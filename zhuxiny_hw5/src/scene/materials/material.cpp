...
glm::vec3 Material::EvaluateScatteredEnergy(const Intersection &isx, const glm::vec3 &woW, const glm::vec3 &wiW, BxDFType flags) const
{
    ...
    return bxdfs[0]->EvaluateScatteredEnergy(wo,wi);
...
/*
    glm::mat4 tran_m1=glm::mat4(glm::vec4(isx.tangent,0),
                                glm::vec4(isx.bitangent,0),
                                glm::vec4(isx.normal,0),
                                glm::vec4(0,0,0,1));*/
...
Intersection Material::SampleLight(float x,float y,Geometry* geo)const
{
    //->EvaluateScatteredEnergy(woW,wiW);
    return Intersection();
}
