...
void Triangle::ComputeArea()
{
    ...
    glm::vec3 p01=points[0] - points[1];
    glm::vec3 p21=points[2]- points[1];

    area= glm::length(glm::cross(p01, p21)) * 0.5;

}
glm::vec3 Triangle:: SetRandomPoint(glm::vec3& local_p,float x,float y)
{
    float q1=x/2.0;
    float q2=y/2.0;
    float q3=1-q1-q2;
    glm::vec3 tpoint = points[0]*q1 + points[1]*q2+ points[2]*q3;
    return tpoint;
...
glm::vec3 Mesh:: SetRandomPoint(glm::vec3& local_p,float x,float y)
{
    int t = x*faces.size();
    glm::vec3 xx;
    glm::vec3 point = faces[t]->SetRandomPoint(xx,x,y);//tranform2??
    glm::vec4 tpoint = transform.T()*glm::vec4(point,1.0);
    return glm::vec3(tpoint);
