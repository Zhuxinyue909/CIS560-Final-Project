...
glm::vec3 Sphere:: SetRandomPoint(glm::vec3& local_p,float x,float y)
{
    float sx=0.5*sin(x*PI)*cos(y*PI*2.0);
    float sy=0.5*sin(x *PI)*sin(y*2.0*PI);
    float sz=0.5*cos(x*PI);
    glm::vec4 temp=transform.T()*glm::vec4(sx,sy,sz,1.0);
    //glm::vec3 offset=glm::vec3(temp)-glm::vec3(0.1,0.1,0.1);
...
Intersection Sphere::GetIntersection(Ray r)
{
    ...
    if(t >= 0)
    {
        ...
        glm::vec3 tangent = glm::cross(glm::vec3(0,1,0),glm::normalize(glm::vec3(P)));//010,normal
